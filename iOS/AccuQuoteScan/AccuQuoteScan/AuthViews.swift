...
// MARK: - Auth Gate
// Sits between @main and ContentView. Routes to LoginView when signed out.
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 24) {
                ...
                .frame(maxWidth: .infinity)
                .frame(height: 50)
                .background(email.isEmpty ? AQ.blue.opacity(0.4) : AQ.blue)
                .cornerRadius(12)
...
struct PaywallSheet: View {
    ...
    private func subscribe(tier: EntitlementManager.SubscriptionTier) {
        guard let token = auth.userId.isEmpty ? nil : auth.userId else { return }
        isLoading = true
        error = nil
        Task {
            defer { isLoading = false }
            guard let idToken = await AuthManager.shared.currentIdToken(),
                  let url = URL(string: "\(AQBackend.baseURL)/api/stripe/create-checkout") else {
            ...
            guard let (data, _) = try? await URLSession.shared.data(for: req),
                  ...
                  let openURL = URL(string: checkoutURL) else {
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14)
                .stroke(isCurrentTier ? AQ.blue.opacity(0.4) : AQ.rule, lineWidth: isCurrentTier ? 2 : 1)
        )
    }
}

// MARK: - Locked Result View (free tier gate)
...
struct LockedResultView: View {
    ...
    var body: some View {
        ZStack {
            // Show the dimensions summary (free)
            ScrollView {
                VStack(spacing: 20) {
                    // Room dimensions card — same style as ResultView
                    DimensionsSummaryCard(result: result)
                        .padding(.horizontal, 20)
                        .padding(.top, 24)

                    // Locked quote section
                    VStack(spacing: 16) {
...
// MARK: - Dimensions Summary Card (shared between LockedResultView and full ResultView)
